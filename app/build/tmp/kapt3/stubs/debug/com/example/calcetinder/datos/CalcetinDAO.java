...
public abstract interface CalcetinDAO {
    ...
    @androidx.room.Query(value = "SELECT DISTINCT calcetines.* FROM calcetines INNER JOIN matches ON calcetines.id = matches.calcetinId WHERE matches.usuarioId = :usuarioId AND matches.tipoMatch = \'like\'")
    @org.jetbrains.annotations.NotNull()
    public abstract kotlinx.coroutines.flow.Flow<java.util.List<com.example.calcetinder.modelo.Calcetin>> obtenerCalcetinesLikeados(int usuarioId);
}